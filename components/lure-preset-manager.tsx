...
import { addLurePreset, removeLurePreset } from '@/lib/actions/trip-mode'

export function LurePresetManager({ presets }: { presets: string[] }) {
  ...
  async function handleAdd() {
    ...
    setAdding(true)
    await addLurePreset(val)
    setList(prev => [...prev, val])
    setInput('')
    setAdding(false)
  }

  async function handleRemove(lure: string) {
    await removeLurePreset(lure)
    setList(prev => prev.filter(s => s !== lure))
  }
  ...
  return (
    <div className="space-y-3">
      {list.length > 0 ? (
        ...
        <p className="text-sm text-slate-400">No lures added yet. Add what you commonly fish with.</p>
      )}
      ...
      <div className="flex gap-3 pt-1">
        <input
          ...
          placeholder="e.g. Chatterbait, Crankbait, Live Shad"
