...
export const TOUR_STEPS = [
  ...
  { path: '/clients',     selector: '[data-tour="clients-content"]', title: 'Client Management',            description: "Every client's contact info, full trip history, and outstanding balance — organized and always with you. Tap any client to call, email, or view their record.",                                                                          position: 'bottom' },
  { path: '/analytics',   selector: '[data-tour="analytics-tabs"]',  title: 'Analytics',                   description: 'Switch between Fishing and Financials. Fishing shows avg fish per trip, best day of the week, and top spots. Financials tracks your revenue, collection rate, and year-over-year growth.',                                               position: 'bottom' },
  { path: '/water-flows', selector: '[data-tour="water-flows-content"]', title: 'Water Flows',             description: 'Live USGS river flow data for any gauge in the US. Search for your local rivers, save your favorites, and check conditions before every trip. Never show up to blown-out water again.',                                                  position: 'bottom' },
]

interface SpotlightRect { top: number; left: number; width: number; height: number }

function getTooltipPos(rect: SpotlightRect, position: string, wH: number, wW: number) {
  const pad = 16, tW = 310, tH = 200
  const pos = position === 'bottom' ? (wH - rect.top - rect.height > tH + pad ? 'bottom' : 'top')
            : position === 'top'    ? (rect.top > tH + pad ? 'top' : 'bottom')
            : position
  const top  = pos === 'bottom' ? rect.top + rect.height + pad : rect.top - tH - pad
  const left = Math.min(Math.max(rect.left + rect.width / 2 - tW / 2, pad), wW - tW - pad)
  return { top, left, arrow: pos === 'bottom' ? 'top' : 'bottom' }
}
...
// Always mounted — manages its own active state so it never loses step on navigation
export function SpotlightTour({ onDone }: { onDone: () => void }) {
  const router   = useRouter()
  const pathname = usePathname()
  const retryRef = useRef<ReturnType<typeof setTimeout> | null>(null)

  // active is false until useEffect confirms on the client
  ...
  // Check on client only — avoids hydration mismatch
  useEffect(() => {
    if (localStorage.getItem(DONE_KEY) !== '1') setActive(true)
  }, [])
  ...
  // Activate step whenever step or pathname changes
  useEffect(() => {
  ...
  if (showWelcome) {
    return <WelcomeCard onStart={() => setShowWelcome(false)} onSkip={finish} />
  }
  ...
  const { top: ttTop, left: ttLeft, arrow } = getTooltipPos(hl, current.position ?? 'bottom', wH, wW)
  ...
  return (
    <div className="fixed inset-0 z-50 pointer-events-none">
      ...
      <div className="absolute bg-white rounded-2xl shadow-2xl pointer-events-auto" style={{ top: ttTop, left: ttLeft, width: 310, zIndex: 52 }} onClick={e => e.stopPropagation()}>
        {arrow === 'top'    && <div className="absolute -top-2    left-1/2 -translate-x-1/2 w-4 h-4 bg-white rotate-45 rounded-sm shadow-sm" />}
        {arrow === 'bottom' && <div className="absolute -bottom-2 left-1/2 -translate-x-1/2 w-4 h-4 bg-white rotate-45 rounded-sm shadow-sm" />}
        ...
        <div className="p-5">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => { if (isLast) { finish(); router.push('/dashboard') } else setStep(s => s + 1) }} className="flex-1 bg-sky-500 hover:bg-sky-400 text-white font-bold py-2.5 rounded-xl text-sm">
              {isLast ? 'Done ✓' : 'Next →'}
            </button>
          </div>
        </div>
