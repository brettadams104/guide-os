        ...
        <form onSubmit={handleSave} className="space-y-3">
          ...
          <div className="flex gap-2">
            <button type="submit" disabled={saving}
              className="flex-1 bg-sky-500 hover:bg-sky-400 text-white text-xs font-semibold py-2 rounded-lg disabled:opacity-50 transition-colors">
              {saving ? 'Saving...' : 'Save'}
            </button>
            <button type="button" onClick={() => { setEditing(false); setError(null) }}
              className="flex-1 border border-slate-200 text-xs font-medium py-2 rounded-lg hover:bg-slate-50 transition-colors">
              Cancel
            </button>
          </div>
        </form>
  ...
  return (
    <li className="flex items-center justify-between bg-slate-50 rounded-xl px-4 py-2.5">
      ...
      <div className="flex items-center gap-3">
        <button onClick={openEdit} className="text-xs text-sky-500 hover:text-sky-600 font-medium">Edit</button>
